...
public:
	...
	{
		float* encodedInputs = new float[(LayerNeuronCounts[bottleneckLayer]+1) * inputCount];
		...
		for (int inputInd = 0; inputInd < inputCount; inputInd++) 
		{
			...
			if (bottleneckLayer == 0)
			{
				for (int ni = 0; ni < LayerNeuronCounts[0]; ni++)
				{
					...
					encodedInputs[inputInd + ni * inputCount] = fnet[indeks];
			...
			encodedInputs[inputInd + LayerNeuronCounts[bottleneckLayer] * inputCount] = 
