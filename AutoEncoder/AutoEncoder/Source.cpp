...
int main()
{
	...
	std::ifstream fileMultilayer("weight_values(multilayer),e500,64,10.txt");
...
float* compressedInputs = autoencoder.compressInputs();
...
for (int i = inpindeks; i < inpindeks+1; i++)
{
    ...
    std::cout << compressedInputs[i + 500 * inputCount] << "\n\n";
}
